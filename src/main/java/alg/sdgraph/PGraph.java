...
public class PGraph {
	...
	String verifyFile;
	String outputFile;
	final int cutting_window = 10 * 100 * 1000 * 1; // 10ms; 1s is not good
	final int time_sensitive_window = 10 * 1000 * 10; // 10ms

	public PGraph(String path, String verify, String out){
		this.trainFile = path;
	...
	public void train() throws IOException {
		...
		/*
		for (long blockId : this.edges.keySet()){
			TreeSet<Edge> neighbors = this.edges.get(blockId);
			int total = 0;
			for (Edge e : neighbors){
				total += e.getWeight();
			}
			for (Edge e : neighbors){
				e.prob = (double) e.weight / (double) total;
			}
		}
		*/
	...
	public void accuracy(int number) throws IOException {
		...
		for (int k = 0; k < lines.size(); k++) {
			...
			String[] ioaccess = line.split("\\s");
			String[] iotype = op.split("\\s");

			for (int i = 0; i < ioaccess.length; i++) {
				String type = iotype[i];
				String io = ioaccess[i];
				...
				if (io.equalsIgnoreCase(""))
					continue;
				long ioId = Long.parseLong(io);

				//build predict
				ArrayList<Long> predicted = new ArrayList<>();
				if (this.edges.containsKey(ioId)){
					ArrayList<Edge> neighbors = this.edges.get(ioId);
					for (Edge e : neighbors){
						if (predicted.size() < number)
							predicted.add(e.dst);
					}
				}
				if ((i + 1) < ioaccess.length) {
					...
					if (predicted.contains(nextId)) {
						hit++;
					} else {
						if (sequencialMap.containsKey(ioId))
							if (nextId > ioId && nextId < (ioId + sequencialMap.get(ioId)))
								hit++;
					}
				}
			}
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		PGraph pg = new PGraph(args[0], args[1], args[2]);
		pg.loadTrainFile();
